#include "frc/control_loops/drivetrain/localization/puppet_localizer.h"
...
DrivetrainConfig<double> GetTestPuppetDrivetrainConfig() {
  DrivetrainConfig<double> config = test_robot::GetDrivetrainConfig();
  return config;
}
}  // namespace
...
using frc::control_loops::drivetrain::DrivetrainLoop;
using frc::control_loops::drivetrain::testing::DrivetrainSimulation;
...
class LocalizedDrivetrainTest : public frc::testing::ControlLoopTest {
 protected:
  LocalizedDrivetrainTest()
      : frc::testing::ControlLoopTest(
        ...
        localizer_output_sender_(
            imu_test_event_loop_->MakeSender<frc::controls::LocalizerOutput>(
                "/localizer")),
        drivetrain_goal_fetcher_(
            test_event_loop_->MakeFetcher<Goal>("/drivetrain")),
        drivetrain_status_fetcher_(
            test_event_loop_
                ->MakeFetcher<frc::control_loops::drivetrain::Status>(
                    "/drivetrain")),
  ...
  aos::Sender<frc::controls::LocalizerOutput> localizer_output_sender_;
  ...
  void SendGoal(double left, double right) {
    ...
    drivetrain_builder.add_controller_type(
        frc::control_loops::drivetrain::ControllerType::MOTION_PROFILE);
