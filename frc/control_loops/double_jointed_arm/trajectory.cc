...
::Eigen::Matrix<double, 2, 6> TrajectoryFollower::K_at_state(
  ...
  const ::Eigen::Matrix<double, 4, 4> final_A =
      ::frc::control_loops::NumericalJacobianX<4, 2, double>(
  ...
  const ::Eigen::Matrix<double, 4, 2> final_B =
      ::frc::control_loops::NumericalJacobianU<4, 2, double>(
          [this](const auto &x_blocked, const auto &U, double dt) {
            return this->dynamics_->UnboundedDiscreteDynamics(x_blocked, U, dt);
          },
          x_blocked, U, 0.00505);

  if (auto sub_K = ::frc::controls::dlqr<4, 2>(final_A, final_B, Q, R)) {
...
::Eigen::Matrix<double, 2, 1> TrajectoryFollower::PlanNextGoal(
  ...
  ::Eigen::Matrix<double, 2, 1> next_goal = ::frc::control_loops::RungeKutta(
