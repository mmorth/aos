#ifndef FRC_CONTROL_LOOPS_HYBRID_STATE_FEEDBACK_LOOP_H_
#define FRC_CONTROL_LOOPS_HYBRID_STATE_FEEDBACK_LOOP_H_
...
class StateFeedbackHybridPlant {
 ...
 private:
  void UpdateAB(::std::chrono::nanoseconds dt) {
    ::frc::controls::C2D(coefficients().A_continuous,
...
class HybridKalman {
 ...
 private:
  void UpdateQR(
      ...
      ::std::chrono::nanoseconds dt) {
    frc::controls::DiscretizeQ(Q_continuous, plant->coefficients().A_continuous,
                               dt, &Q_);
...
#endif  // FRC_CONTROL_LOOPS_HYBRID_STATE_FEEDBACK_LOOP_H_
