...
Imu::Imu(aos::ShmEventLoop *event_loop, double encoder_scalar)
  ...
  epoll->OnReadable(imu_fd_, [this]() {
    ...
    const flatbuffers::Offset<frc::IMUValues> values_offset =
        ProcessReading(sender.fbb(), absl::Span(buf, kReadSize));
    const flatbuffers::Offset<
        flatbuffers::Vector<flatbuffers::Offset<frc::IMUValues>>>
        readings_offset = sender.fbb()->CreateVector(&values_offset, 1);
    frc::IMUValuesBatch::Builder batch_builder =
        sender.MakeBuilder<frc::IMUValuesBatch>();
    batch_builder.add_readings(readings_offset);
    imu_sender_.CheckOk(sender.Send(batch_builder.Finish()));
  });
}

flatbuffers::Offset<frc::IMUValues> Imu::ProcessReading(
...
flatbuffers::Offset<frc::ADIS16470DiagStat> Imu::PackDiagStat(
