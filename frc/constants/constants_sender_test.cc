...
namespace frc::constants::testing {
 ...
 public:
  ConstantSenderTest()
      : config_(aos::configuration::ReadConfig(
            ArtifactPath("frc/constants/testdata/aos_config.json"))),
...
// For team 4646, compares the data that is recived from the program, to the
// data that is expected

TEST_F(ConstantSenderTest, HasData4646) {
  aos::network::OverrideTeamNumber(4646);
...
// For team 118, compares the data that is recived from the program, to the
// data that is expected.

TEST_F(ConstantSenderTest, HasData118) {
  std::unique_ptr<aos::EventLoop> test_event_loop =
      event_loop_factory_.MakeEventLoop("constants");
