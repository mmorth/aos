...
class CANTranslatorTest : public ::testing::Test {
 public:
  CANTranslatorTest()
      : config_(aos::configuration::ReadConfig(
        ...
        dual_imu_fetcher_(
            can_translator_event_loop_->MakeFetcher<frc::imu::DualImu>("/imu")),
        can_translator_status_fetcher_(
            can_translator_event_loop_
                ->MakeFetcher<frc::imu::CanTranslatorStatus>("/imu")),
        can_frame_sender_(
            can_frame_event_loop_->MakeSender<frc::can_logger::CanFrameStatic>(
                "/can")),
 ...
 protected:
  ...
  aos::Fetcher<frc::imu::DualImu> dual_imu_fetcher_;
  aos::Fetcher<frc::imu::CanTranslatorStatus> can_translator_status_fetcher_;

  aos::Sender<frc::can_logger::CanFrameStatic> can_frame_sender_;

  frc::imu_fdcan::CANTranslator can_translator_;
};
...
TEST_F(CANTranslatorTest, CheckValidFrame) {
  ...
  can_frame_event_loop_->OnRun([this] {
    ...
    for (size_t i = 0; i < 8; ++i) {
      aos::Sender<frc::can_logger::CanFrameStatic>::StaticBuilder
  ...
  EXPECT_EQ(dual_imu_fetcher_->kernel_timestamp(), 649);
}
