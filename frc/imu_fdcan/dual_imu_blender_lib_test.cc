...
class DualImuBlenderTest : public ::testing::Test {
  ...
  DualImuBlenderTest()
      : config_(aos::configuration::ReadConfig(
        ...
        imu_values_batch_fetcher_(
            dual_imu_event_loop_->MakeFetcher<frc::IMUValuesBatch>(
                "/localizer")),
        dual_imu_blender_status_fetcher_(
            dual_imu_blender_event_loop_
                ->MakeFetcher<frc::imu::DualImuBlenderStatus>("/imu")),
        dual_imu_sender_(
            dual_imu_event_loop_->MakeSender<frc::imu::DualImuStatic>("/imu")),
        dual_imu_blender_(dual_imu_blender_event_loop_.get()) {}

  void CheckImuType(frc::imu::ImuType type) {
 ...
 protected:
  ...
  aos::Fetcher<frc::IMUValuesBatch> imu_values_batch_fetcher_;
  aos::Fetcher<frc::imu::DualImuBlenderStatus> dual_imu_blender_status_fetcher_;

  aos::Sender<frc::imu::DualImuStatic> dual_imu_sender_;

  frc::imu_fdcan::DualImuBlender dual_imu_blender_;
};
...
TEST_F(DualImuBlenderTest, SanityCheck) {
  dual_imu_blender_event_loop_->OnRun([this] {
    aos::Sender<frc::imu::DualImuStatic>::StaticBuilder dual_imu_builder =
        dual_imu_sender_.MakeStaticBuilder();

    frc::imu::SingleImuStatic *murata = dual_imu_builder->add_murata();

    auto *murata_chip_states = murata->add_chip_states();
    frc::imu::ChipStateStatic *murata_uno_chip_state =
        murata_chip_states->emplace_back();
    frc::imu::ChipStateStatic *murata_due_chip_state =
        murata_chip_states->emplace_back();

    frc::imu::SingleImuStatic *tdk = dual_imu_builder->add_tdk();
  ...
  CheckImuType(frc::imu::ImuType::MURATA);
}
...
TEST_F(DualImuBlenderTest, Saturation) {
  ...
  CheckImuType(frc::imu::ImuType::TDK);
