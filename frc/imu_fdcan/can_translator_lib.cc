...
CANTranslator::CANTranslator(aos::EventLoop *event_loop,
    ...
    : event_loop_(event_loop),
      dual_imu_sender_(event_loop->MakeSender<frc::imu::DualImuStatic>("/imu")),
      can_translator_status_sender_(
          event_loop->MakeSender<frc::imu::CanTranslatorStatusStatic>("/imu")) {
  packets_arrived_.fill(false);
  // TODO(max): Update this with a proper priority
  event_loop->SetRuntimeRealtimePriority(58);

  event_loop->MakeWatcher(
      canframe_channel, [this](const frc::can_logger::CanFrame &can_frame) {
  ...
  event_loop->AddPhasedLoop(
      [this](int) {
        aos::Sender<frc::imu::CanTranslatorStatusStatic>::StaticBuilder
...
void CANTranslator::HandleFrame(const frc::can_logger::CanFrame *can_frame) {
  ...
  frc::imu::ChipStateStatic *tdk_chip_state = tdk_chip_states->emplace_back();
