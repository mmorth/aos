...
using frc::imu_fdcan::DualImuBlender;

DualImuBlender::DualImuBlender(aos::EventLoop *event_loop)
    : imu_values_batch_sender_(
          event_loop->MakeSender<frc::IMUValuesBatchStatic>("/localizer")),
      dual_imu_blender_status_sender_(
          event_loop->MakeSender<frc::imu::DualImuBlenderStatusStatic>(
              "/imu")) {
  // TODO(max): Give this a proper priority
  event_loop->SetRuntimeRealtimePriority(15);

  event_loop->MakeWatcher("/imu", [this](const frc::imu::DualImu &dual_imu) {
    HandleDualImu(&dual_imu);
  });
}

void DualImuBlender::HandleDualImu(const frc::imu::DualImu *dual_imu) {
  aos::Sender<frc::IMUValuesBatchStatic>::StaticBuilder
      imu_values_batch_builder_ = imu_values_batch_sender_.MakeStaticBuilder();

  aos::Sender<frc::imu::DualImuBlenderStatusStatic>::StaticBuilder
