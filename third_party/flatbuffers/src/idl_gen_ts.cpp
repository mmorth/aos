  ...
  void generateEntry() {
    ...
    for (const auto &it : ns_defs_) {
      ...
      for (const auto &def : it.second.definitions) {
        ...
        // AOS modification: For the location where this generated file ends up,
        // the prior logic was straightforwardsly wrong and added too many
      ...
      for (const auto &it2 : ns_defs_) {
        ...
        // AOS modification: This was not the correct relative path; we
        // were not exercising this logic, so omit these exports.
        //std::string rel_path = it2.second.path;
        // AOS modification: ts_no_import_ext was not being respected.
        //code += rel_path + (parser_.opts.ts_no_import_ext ? "';\n" : ".js';\n");
        //export_counter++;
      }
